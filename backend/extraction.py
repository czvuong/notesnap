...
from cost_tracking import log_cost
...
# ── OCR result cache ──────────────────────────────────────────────────────────
# Maps SHA-256(image_bytes) → raw OCR text.
# Skips one full API round-trip when the same image is uploaded more than once.
# Uses an OrderedDict so we can evict the oldest entry (FIFO) when full.

_OCR_CACHE_MAX = 200                          # max unique images to remember
_ocr_cache: OrderedDict[str, str] = OrderedDict()


def _ocr_cache_get(image_bytes: bytes) -> tuple[str, str | None]:
    """Return (sha256_hex, cached_text_or_None)."""
    key = hashlib.sha256(image_bytes).hexdigest()
    return key, _ocr_cache.get(key)


def _ocr_cache_put(key: str, raw_text: str) -> None:
    """Insert into cache, evicting the oldest entry if at capacity."""
    if key in _ocr_cache:          # move to end (most-recently used)
        _ocr_cache.move_to_end(key)
    else:
        if len(_ocr_cache) >= _OCR_CACHE_MAX:
            _ocr_cache.popitem(last=False)   # drop oldest
...
def extract_note(
    ...
    # Step 0: hash the raw bytes — used for duplicate detection and OCR cache
    ...
    if provider == "tritonai":
        ...
        if is_pdf:
            ...
            # Step 3a: OCR — image → raw text
            # Check the in-process cache first; if the same image was uploaded
            # before we skip the API call entirely and go straight to structuring.
            cache_key, cached_text = image_hash, _ocr_cache.get(image_hash)
            if cached_text is not None:
                logger.info("OCR cache hit (sha256=%s…) — skipping API call", cache_key[:12])
                ...
                try:
                    ...
                    _ocr_cache_put(cache_key, raw_text)
                    ocr_used = True
    ...
    # Step 4: parse, attach hash, and return
    result = _parse_ai_response(raw_json, mode, model_name)
...
def _call_ocr_model(b64_image: str, mime_type: str) -> str:
    ...
    response = client.chat.completions.create(
        ...
        # Keep at 4096: api-lightonocr-1b total context window is 8192 tokens,
        # so output cannot be set to 8192 when there is any input.
    )
    if response.usage:
        log_cost(
            model=settings.TRITONAI_OCR_MODEL,
