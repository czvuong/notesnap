...
Scoped to the current user. Use hard delete (not soft) since they are preferences.
"""
...
def list_corrections(
    ...
    corrections = db.query(Correction).filter(
        Correction.user_id == current_user
...
def update_correction(
    ...
    correction = _get_or_404(db, correction_id, current_user)
...
def _get_or_404(db: Session, correction_id: str, user_id: str) -> Correction:
    c = db.query(Correction).filter(
        Correction.id == correction_id, Correction.user_id == user_id
    ).first()
    if not c:
