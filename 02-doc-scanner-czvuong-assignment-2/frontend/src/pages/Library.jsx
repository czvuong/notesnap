...
import { listNotes, listCourses, listTags, createCourse } from '../api.js'
import './Library.css'
...
export default function Library() {
  ...
  }, [q, courseId, tag, mode, sort, page]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function clearFilters() {
    setQ(''); setCourseId(''); setTag(''); setMode(''); setSort('newest'); setPage(1)
  }

  const totalPages  = Math.max(1, Math.ceil(total / PAGE_SIZE))
  const hasFilters  = q || courseId || tag || mode || sort !== 'newest'
  const filterLabel = [
    q        && `"${q}"`,
