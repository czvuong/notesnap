...
- **Public shareable note links: Opt-in toggle that generates a read-only public URL to share a note (accessible without login)**
  > ✅ Implemented, but changed. Backend fully implemented: `is_public` boolean and `public_slug` field added to the `Note` table via inline raw SQL migration, `PATCH /api/notes/{id}/share` toggles public status and generates a slug, and an unauthenticated `GET /api/notes/public/{slug}` route serves the read-only view rendered by `frontend/src/pages/PublicNoteView.jsx`. The UI toggle was intentionally removed from the Share modal to avoid confusion with the new invite-based collaborative sharing (Feature 3). Public links can still be toggled via the API; a UI entry point was descoped.

- **Mobile-responsive UI: Add responsive breakpoints for phone-sized screens**
  > ✅ Implemented. `@media` breakpoints added across all major CSS files: `App.css` (sidebar collapses on small screens), `Library.css`, `NoteEditor.css`, `Upload.css`, `StudyHub.css`, `BatchUpload.css`, `Courses.css`, `Dashboard.css`, and `PublicNoteView.css`.
...
## Features Added Beyond the Original Proposal

These features were not in the original proposal but were built during the project.
...
- `NoteCollaborator` model in `backend/models.py`: stores invitee email, optional resolved `invitee_user_id`, and a `permission` level (`view`, `comment`, or `edit`). Invite matching is two-layer: JWT email claim first, Clerk Backend API fallback via `_fetch_email_from_clerk()` in `backend/auth.py` for users whose JWT does not include an email.
- `NoteComment` model in `backend/models.py`: threaded comments scoped per note and per user, with a `resolved` flag.
- `backend/routers/collaborators.py`: invite by email (`POST /api/notes/{id}/collaborators`), list collaborators, update permission, remove collaborator, and a `/api/me/shared` route that returns notes shared with the current user.
- `backend/routers/sections.py`: rewritten to allow collaborators with `edit` permission to read and write sections; view/comment collaborators get read-only access.
- `frontend/src/pages/NoteEditor.jsx`: Share modal with invite flow, collaborator list with per-collaborator permission dropdowns (`PermSelect` component), and permission-aware rendering throughout the editor (`isOwner`, `canEdit`, `canComment` booleans).
- `frontend/src/pages/Library.jsx`: "Shared with me" section below the main notes grid using `SharedNoteCard` components, fetching from `/api/me/shared`.
